...
function Basket() {
  ...
  return (
    <div className="w-full flex justify-center gap-x-5">
      <div className="w-[75%] ml-5 min-h-[300px] my-10 bg-white border border-blue-200 rounded-md">
        {!complete ? (
          <>
            {basket.length > 0 ? (
              basket.map((item, idx) => (
                <div
                  key={idx}
                  className="w-full my-4 p-2 flex justify-between items-center"
                >
                  <div className="w-[30%] h-[150px] object-cover border border-zinc-300 rounded-lg overflow-hidden">
                    <img
                  ...
                  <div className="w-[20%]">
                    <div className="w-[100px] h-10 border rounded-md flex justify-center items-center border-zinc-300">
                      <button
                  ...
                  <p className="w-[20%] text-right text-blue-900">
                    ${item.price.toFixed(2)}
                  </p>
                  <div className="w-[10%] flex justify-center">
                    <button
                      onClick={() => dispatch(removeItem(item.id))}
                      className="p-1 rounded-full flex justify-center text-xl text-red-700 hover:bg-red-100 transition-all"
                    >
                      <MdDelete />{" "}
                    </button>
                  </div>
        ...
        {basket.length > 0 && (
          <div className="flex justify-end mr-2 gap-x-2 my-2">
            <button
      ...
      <div className="w-[25%] mr-5 p-3 h-[300px] mt-10 bg-white border border-blue-200 rounded-md">
        <div className="mt-1">
      ...
      {sneakbar && (
        <SneakBar
          type="complete"
          message="Siparişiniz başarıyla gerçekleşti..."
        />
      )}
    </div>
  );
